...
To further explore the dataset, I found that all the data were collected from `r length(unique(flow_sample$station_id))` monitoring station. There were 2 types of extremes were recorded. And the number of unique values of the year and month variables were 109 and 11 respectively.
...
<!-------------------------- Start your work below ---------------------------->
One of the research questions that I would like to answer is how trees in different neighborhoods differ in diameter.
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
Before doing other things, I first investigated how many missing values there are per variable. As we can see from both the table and plot, five out of `r ncol(vancouver_trees)` variables have missing values, and over 50% of date_planted is missing. However, both diameter and neighbourhood_name do not have missing values.
```{r missing values}
...
One of the alternatives to see the distribution a bit clearer would be filtering out observations whose diameter is larger than 40 cm before making a boxplot. Because we can see from both the histogram and the boxplot above that observations with diameter larger than 40 cm is only a very small porportion of our data, excluding them should not cause a problem when our purpose is to see the distribution a bit more clearly.

As the boxplot below shows, we can now see the distribution more clearly.  
```{r Filtering and plot again}
...
I think diameter should be related to height for trees, so I explored the relationship between them using boxplots. Again, I excluded the observations with diameter larger than 40 so that we could see patterns more clearly. As we can see from the plot, there must be a positive association between diameter and height for trees. 
```{r}
...
Since I am also interested to see how species richness (i.e. number of species) differs among neighborhoods, I used the summarise() function to compute species richness for each neighborhoods and visualize it using a bar graph. As we can see from the barplot, Downtown has the lowest species richness.
```{r species richness}
# EX 4.Explore the relationship between 2 variables in a plot.
...
<!--- *****START HERE***** --->
* How is the height of trees related to their age and the usage of root barrier?
* How is species richness in different neighborhoods related to the total number of trees in the neighborhood?
* What is the relationship between diameter and height? How much variation in diameter can be explained by height_range_id?
